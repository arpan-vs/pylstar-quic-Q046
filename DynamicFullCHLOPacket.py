...
{
    'name': b'PAD',
    'value': '2d'*(556 - len(SessionInstance.get_instance().destination_ip) - (len(SessionInstance.get_instance().cached_certificate)//2))
},
{
    'name': b'SNI',
...
{
    'name': b'STK',
    'value': SessionInstance.get_instance().source_address_token 
},
{
    'name': b'SNO',
    'value': SessionInstance.get_instance().server_nonce_initial 
},
{
    'name': b'VER',
    'value': (b'Q046').hex()
...
{
    'name': b'NONC',
    'value': SessionInstance.get_instance().client_nonce
},
{
    'name': b'AEAD',
...
{
    'name': b'SCID',
    'value': SessionInstance.get_instance().server_config_id 
},
{
    'name': b'TCID',
    'value': "00"*4
...
{
    'name': b'PUBS',
    'value': SessionInstance.get_instance().public_values_bytes
},
{
    'name': b'MIDS',
    'value': '64000000'
...
{
    'name': b'XLCT',
    'value': SessionInstance.get_instance().expected_leaf_certificate
},
{
    'name': b'CSCT',
    'value': ''  # C25519
...
{
    'name': b'CCRT',
    'value': SessionInstance.get_instance().cached_certificate
},
{
    'name': b'CFCW',
    'value': '0000f000'
},
{
    ...
    def set_emptyCERT_Hash(self):

        for tag in self.__tags:
            if tag['name'] == b'XLCT':
                tag.update({'value':'00'*8})

    def set_removedCERT_Hash(self):
    ...
    def build_body(self):
        ...
        self.__write_to_body("a4")  # Crypto
        ...
        for tag in self.__tags:
            ...
            # print(tag['value'])
        self.__write_to_body("00"*310)
        return self.__body.hex()
   
    def fuzz_random_tag(self):
        print("InvalidZeroRTT")
    ...
    def build_packet(self, InvalidPacket = False, Fuzz = False):
        ...
        if not (InvalidPacket == False or Fuzz):
            self.set_emptyCERT_Hash()
    ...
    def CHLO_value(self):
        return self.__body[6 : 6 + 1024].hex()
